...
double maxSpeed = 0.3;
double distanceConst = 0.1;
double dt = 0.01, maxT = M_PI, minT = -M_PI, Kp = 0.3, Ki = 0.005, Kd = 0.07;
double dtS = 0.01, maxS = maxSpeed, minS = 0.0, KpS = 0.19, KiS = 0.06, KdS = 0.04;
...
void displayLane(bool isTrajectoryPushed, Geometry &geometry) {
    ...
    std::ifstream vecquim;
    vecquim.open("/home/jorge/Documents/robotica_tiago/turtlebot_control_pack/src/pid_controller/src/vector.txt");
    if (!vecquim.is_open()){
        std::cout << "Não foi possivel abrir o arquivo." << "\n";
        return;
    }
    getline( vecquim, Confirm, ' ');
    if (Confirm == "position")
        vecquim >> NumVec;   
    while (path.points.size() <= NumVec){
        geometry_msgs::Point p; 

        vecquim >> p.y ;
        p.y = p.y / 100;       //some random circular trajectory, with radius 4, and offset (-0.5, 1, 0)
        vecquim >> p.x ;
        p.x = p.x / 100; 
        p.z = 0;
        std::cout << p.x << " " << p.y << "\n";
